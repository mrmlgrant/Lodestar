...
const Cloud = (() => {
  ...
  function signOut() { if (token && libReady()) try { google.accounts.oauth2.revoke(token, () => {}); } catch (e) {} token = null; exp = 0; fileId = null; folderId = null; }
...
function SyncPill({ status }) {
  const map = {
    ...
    saving: ["saving", "Saving…"], saved: ["saved", "Saved to Drive"], offline: ["offline", "Offline"],
  };
  const [cls, label] = map[status] || ["", ""];
  return <span className={`sync ${cls}`}><span className="dot" />{label}</span>;
}
...
function App() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      else { const empty = { version: 1, settings: store.data.settings || {}, trips: [] }; store.setData(empty); await Cloud.save(empty); }
      setStatus("saved");
    } catch (e) { setStatus("offline"); toast("Couldn't reach Drive", true); }
  }, [store, toast]);
  ...
  const signIn = async () => {
    try { setStatus("saving"); const p = await Cloud.signIn(); setUser(p); await loadData(); toast("Signed in"); }
    ...
    <StoreCtx.Provider value={{ ...store, _drag, _dragPlace }}>
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          {view === "trips"
            ? <div>
                {!Cloud.configured() && <div className="content" style={{ paddingBottom: 0 }}><div className="signin-banner"><Ic name="compass" size={22} /><div className="txt"><b>Demo mode.</b><p>Add your Google client ID in the code to save trips to Drive. For now, changes live only in this session.</p></div></div></div>}
                {Cloud.configured() && !user && <div className="content" style={{ paddingBottom: 0 }}><div className="signin-banner"><Ic name="compass" size={22} /><div className="txt"><b>Sign in to save your trips.</b><p>Lodestar keeps everything in your own Google Drive.</p></div><GoogleBtn onClick={signIn}>Sign in with Google</GoogleBtn></div></div>}
